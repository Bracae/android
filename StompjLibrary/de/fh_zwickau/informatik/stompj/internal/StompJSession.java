...
public class StompJSession {
	...
	public ErrorMessage connect() throws StompJException {
		try {
			...
			this.disconnect();
			this.connection.setErrorFlag();
			if (!this.sendExceptionAsErrorMessage(e)
					&& !this.connection.isPollingFlagSet()) {
	 ...
	 */
	boolean sendExceptionAsErrorMessage(final Exception exception) {
		...
		if (this.getConnection().getErrorHandler() != null) {
			this.getConnection().getErrorHandler().onError(error);
			return true;
		} else {
			return false;
		}
